...
#I ZAPYTANIE
SELECT COUNT(*) FROM geo.Milion INNER JOIN GeoTabela ON
(mod(geo.Milion.liczba,102)=(GeoTabela.id_pietro));

#II ZAPYTANIE

SELECT COUNT(*) FROM geo.Milion INNER JOIN geo.Pietro ON
(mod(geo.Milion.liczba,102)=geo.Pietro.id_pietro) NATURAL JOIN geo.Epoka NATURAL JOIN
geo.Okres NATURAL JOIN geo.Era NATURAL JOIN geo.Eon;

#III ZAPYTANIE
SELECT COUNT(*) FROM geo.Milion WHERE mod(Milion.liczba,102)=
(SELECT id_pietro FROM GeoTabela WHERE mod(Milion.liczba,102)=(id_pietro));

#IV ZAPYTANIE

SELECT COUNT(*) FROM geo.Milion WHERE Milion.liczba % 102 IN
...
CREATE INDEX in_stramilion ON geo.milion(liczba);
